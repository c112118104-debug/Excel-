...
本專案庫包含 4 個基於生產管理標準建構的 Excel 高階邏輯模組。專案擺脫傳統單純試算表數據填寫與基礎運算思維，導入參數解耦、防呆驗證 與 權限控管等高階系統架構思維，展現將工業控制、系統監控與數據邏輯完整落地於 Excel 的系統化設計能力。
...
![TPM 設備損失分類樞紐分析表與雙軸圖](PM%20設備損失分類樞紐分析表.png)
